...
def k_fold_train_and_write(data):
    ...
    for train_idx, test_idx in kf.split(data):
        ...
        for n in args.learning_curve_points:
            ...
            trainer.model.save_pretrained(f'{args.model_save_path}/{len(train.head(n))}_kfold-split={split}_model-name={type(model).__name__}')
            ...
            name = '\t'.join([type(model).__name__, str(split), str(n)])
            display_f1(name, df)
            with open(f'{args.prediction_save_path}/{len(train.head(n))}_kfold-split={split}_model-name={type(model).__name__}.tsv','w+') as f:
...
if __name__ == '__main__':
    ...
    if not args.final:
        ...
        trainer.model.save_pretrained(f'{args.model_save_path}/{len(df)}_final_model-name={type(trainer.model).__name__}')
        
        train_predictions = trainer.predict(train_dataset).predictions.argmax(-1)
        df['train/dev'] = 'train'
        df['predicted-label'] = train_predictions
        df['predicted-label'] = df['predicted-label'].map(converter.idx2lbl)
        with open(f'{args.prediction_save_path}/{len(df)}_final_model-name={type(trainer.model).__name__}.tsv','w+') as f:
            df.drop(['index'], axis=1).to_csv(f, sep='\t', index=False)
